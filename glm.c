...
/* glmEqual: compares two vectors and returns 1 if they are
 * equal (within a certain threshold) or 0 if not. An epsilon
 ...
 */
float*
glmWeldVectors(float* vectors, int* numvectors, float epsilon)
{
  float* copies;
  int   copied;
  int   i, j;
...
glmFirstPass(GLMmodel* model, FILE* file) 
{
  ...
  group = model->groups;
  while(group) {
    group->triangles = (int*)malloc(sizeof(int) * group->numtriangles);
    group->numtriangles = 0;
    group = group->next;
  }
}
 ...
 */
void
glmWriteOBJ(GLMmodel* model, char* filename, int mode)
{
  int    i;
  FILE*     file;
  GLMgroup* group;
 ...
 * GLM_FLAT and GLM_SMOOTH should not both be specified.  */
/*int
glmList(GLMmodel* model, int mode)
{
  int list;
...
glmWeld(GLMmodel* model, float epsilon)
{
  ...
  for (i = 0; i < model->numtriangles; i++) {
    T(i).nindices[0] = (int)vectors[3 * T(i).nindices[0] + 0];
    T(i).nindices[1] = (int)vectors[3 * T(i).nindices[1] + 0];
    T(i).nindices[2] = (int)vectors[3 * T(i).nindices[2] + 0];
  }
  ...
  for (i = 0; i < model->numtriangles; i++) {
    for (j = 0; j < 3; j++) {
      T(i).tindices[j] = (int)vectors[3 * T(i).tindices[j] + 0];
    }
  }
